...
TestName="NVMe_Write_UNC"

dir="$(dirname "$0")"
#echo "$dir"
source "$dir/body_Func.sh"

feature=$( nvme id-ctrl /dev/nvme0n1 -H | grep -i "Write Uncor" | awk '{ print $3 }' )

echo "feature_support:$feature"

if [ $feature == "0" -o $feature == "0x0" ]; then
        echo " Write Uncor is not support"
        exit 1
fi
...
NVMe_IO_Write_UNC()
{
		...
		cmd="nvme write-uncor /dev/nvme0n1 -s $SLBA -c $NLB_size"
